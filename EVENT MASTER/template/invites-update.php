 ...
 if(isset($_POST['update']))
  {
   ...
   if(empty($Country))
   {
 $result = mysqli_query($sql_con, "UPDATE `e_invites` SET `CardName`='$Location' WHERE Sno='$sno'");
 }
 else
 {
     $result = mysqli_query($sql_con, "UPDATE `e_invites` SET `CardName`='$Location',`CardImage`='$Country' WHERE Sno='$sno'");
 }
  }
 if(isset($_POST['delete']))
{
 $sno = isset($_POST['sno'])?$_POST['sno']:null;
$result1 = mysqli_query($sql_con,"DELETE FROM `e_invites` WHERE Sno='$sno'");
...
<body>
  <div class="container-scroller">
        ...
        <div class="content-wrapper">
          ...
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">E-invite Information</h4>
    ...
    <tbody>
        ...
        <?php
$result = mysqli_query($sql_con,"SELECT * FROM `e_invites` where RegistrationId='$id'");
$count = 0;
while($row = mysqli_fetch_array($result))
{
    $sno = $row['Sno'];
  $Inquiryid = $row["RegistrationId"];
  $InquiryName = $row["CardName"];
  $Country = $row["CardImage"];
  ?>
   <tr>
    ...
    <td><?php echo $Country;?></td>
     <td><button type="button" class="btn btn-info" data-toggle="modal" onclick="document.getElementById('sno').value = '<?php echo $sno; ?>';document.getElementById('Register').value = '<?php echo $Inquiryid; ?>';document.getElementById('Location').value = '<?php echo $InquiryName; ?>';document.getElementById('Country').value = '<?php echo $Country; ?>';" data-target="#myMod">Details</button></td>
    </tr>
  <?php
}
       ?>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title">E-Invites Information</h4>
...
<form method="post" enctype="multipart/form-data">
  ...
  <div class="form-group has-error">
    <label for="time1">Card Name</label>
    <input type="text" class="form-control" id="Location" placeholder="Location" name="Location">
  </div>
 
   <div class="form-group has-error">
    <label for="time1">Card Image</label>
