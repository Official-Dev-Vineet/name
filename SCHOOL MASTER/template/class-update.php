 ...
 if(isset($_POST['update']))
  {
   ...
   if(empty($image))
   {
 $result = mysqli_query($sql_con, "UPDATE `topper` SET `Name`='$Register',`Class`='$Location',`Position`='$state',`Percentage`='$Country' WHERE Sno='$sno'");
 }
 else
 {
     $result = mysqli_query($sql_con, "UPDATE `topper` SET `Name`='$Register',`Class`='$Location',`Position`='$state',`Percentage`='$Country',`Image`='$image' WHERE Sno='$sno'");
 }
  }
 if(isset($_POST['delete']))
{
 $sno = isset($_POST['sno'])?$_POST['sno']:null;
$result1 = mysqli_query($sql_con,"DELETE FROM `management` WHERE Sno='$sno'");
...
<body>
  <div class="container-scroller">
    ...
    <tbody>
        ...
        <?php
$result = mysqli_query($sql_con,"SELECT * FROM `topper` where RegistrationId='$id'");
$count = 0;
while($row = mysqli_fetch_array($result))
{
    $sno = $row['Sno'];
  $Inquiryid = $row['Name'];
  $InquiryName = $row["Class"];
  $State = $row["Position"];
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title">Class Topper Information</h4>
...
<form method="post" enctype="multipart/form-data">
  ...
  <div class="form-group has-warning">
    ...
    <input type="text" class="form-control" id="Register" placeholder="" name="Register">
  </div>  
  <div class="form-group has-error">
    <label for="time1">Class</label>
    <input type="text" class="form-control" id="Location" name="Location">
  </div>
   <div class="form-group has-error">
    <label for="time1">Position</label>
    <textarea type="text" class="form-control" id="State" name="State"></textarea>
  </div>
   <div class="form-group has-error">
    <label for="time1">Percentage</label>
