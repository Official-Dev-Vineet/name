 ...
 if(isset($_POST['update']))
  {
   ...
   if(empty($Country))
   {
 $result = mysqli_query($sql_con, "UPDATE `management` SET `Name`='$Register',`Designation`='$Location',`Description`='$state' WHERE Sno='$sno'");
 }
 else
 {
     $result = mysqli_query($sql_con, "UPDATE `management` SET `Name`='$Register',`Designation`='$Location',`Description`='$state',`Image`='$Country' WHERE Sno='$sno'");
 }
  }
...
<body>
  <div class="container-scroller">
    ...
    <tbody>
        ...
        <?php
$result = mysqli_query($sql_con,"SELECT * FROM `management` where RegistrationId='$id'");
$count = 0;
while($row = mysqli_fetch_array($result))
{
    $sno = $row['Sno'];
  $Inquiryid = $row['Name'];
  $InquiryName = $row["Designation"];
