 ...
 if(isset($_POST['update']))
  {
   ...
   if(empty($Location))
   {
    $result = mysqli_query($sql_con, "UPDATE `banner` SET `Banner_two`='$state',`Banner_three`='$Country' WHERE Sno='$sno'");
   }
   else
   {
     $result = mysqli_query($sql_con, "UPDATE `banner` SET `Banner_One`='$Location',`Banner_two`='$state',`Banner_three`='$Country' WHERE Sno='$sno'");  
   }
   
   if(empty($state))
   {
     $result = mysqli_query($sql_con, "UPDATE `banner` SET `Banner_One`='$Location',`Banner_three`='$Country' WHERE Sno='$sno'");  
   }
   else
   {
      $result = mysqli_query($sql_con, "UPDATE `banner` SET `Banner_One`='$Location',`Banner_two`='$state',`Banner_three`='$Country' WHERE Sno='$sno'"); 
   ...
   if(empty($Country))
   {
      $result = mysqli_query($sql_con, "UPDATE `banner` SET `Banner_One`='$Location',`Banner_two`='$state' WHERE Sno='$sno'"); 
   }
   else
   {
 $result = mysqli_query($sql_con, "UPDATE `banner` SET `Banner_One`='$Location',`Banner_two`='$state',`Banner_three`='$Country' WHERE Sno='$sno'");
...
{
 $sno = isset($_POST['sno'])?$_POST['sno']:null;
$result1 = mysqli_query($sql_con,"DELETE FROM `banner` WHERE Sno='$sno'");
...
<body>
  <div class="container-scroller">
    ...
    <tbody>
        ...
        <?php
$result = mysqli_query($sql_con,"SELECT * FROM `banner` where RegistrationId='$id'");
$count = 0;
while($row = mysqli_fetch_array($result))
{
    $sno = $row['Sno'];
  $Inquiryid = $row["RegistrationId"];
  $InquiryName = $row["Banner_One"];
  $State = $row["Banner_two"];
  $Country = $row["Banner_three"];
  ?>
   <tr>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h4 class="modal-title">Banner Information</h4>
...
<form method="post" enctype="multipart/form-data">
  ...
  <div class="form-group has-error">
    <label for="time1">Banner 1</label>
    <input type="file" class="form-control" id="Location" placeholder="Location" name="Location">
  </div>
   <div class="form-group has-error">
    <label for="time1">Banner 2</label>
    <input type="file" class="form-control" id="State" placeholder="State" name="State">
  </div>
   <div class="form-group has-error">
    <label for="time1">Banner 3</label>
