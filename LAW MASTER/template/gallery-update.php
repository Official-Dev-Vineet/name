...
if(isset($_POST['update5']))
  ...
  $result = mysqli_query($sql_con, "UPDATE `gallery` SET `RegistrationId`='$Registera', `Image`='$img' WHERE Sno='$sno'");
}
if(isset($_POST['delete5']))
{
 $Registera = isset($_POST['Registera'])?$_POST['Registera']:null;
$result1 = mysqli_query($sql_con,"DELETE FROM `gallery` WHERE Sno='$sno'");
...
<thead>
    ...
    <tbody>
        ...
        <?php
$result = mysqli_query($sql_con,"SELECT * FROM `gallery` where RegistrationId='$id'");
while($row = mysqli_fetch_array($result))
{
  ...
  ?>
   <tr>
    <td><?php echo $id;?></td>
        <td><?php echo $imag;?></td>
    <td><button type="button" class="btn btn-primary" data-toggle="modal" onclick="document.getElementById('Sno').value = '<?php echo $sno; ?>';document.getElementById('Registera').value = '<?php echo $id; ?>';document.getElementById('image').value = '<?php echo $img; ?>'; " data-target="#myModal8">Details</button></td></tr>
  <?php
}
       ?>
      </tbody>
  ...
  <div class="form-group has-warning">
    ...
    <input type="text" class="form-control" id="Registera" value="<?php echo $id; ?>" placeholder="Registeration Id" name="Registera" readonly>
  </div>  
  ...
  <button type="submit" name="delete5" class="btn btn-info">Delete</button>
</form>
